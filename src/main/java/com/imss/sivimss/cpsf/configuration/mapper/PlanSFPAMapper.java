...
public interface PlanSFPAMapper {
	...
	@Select("SELECT PSFPA.NUM_FOLIO_PLAN_SFPA AS numFolioPlanSFPA, PSFPA.IND_TITULAR_SUBSTITUTO indTitularSubs, DATE_FORMAT(P1.FEC_NAC, '%d-%m-%Y') AS fecNacTitular, "
			+ "CONCAT(IFNULL(P1.NOM_PERSONA,''),' ',IFNULL(P1.NOM_PRIMER_APELLIDO,''), ' ',IFNULL(P1.NOM_SEGUNDO_APELLIDO,'')) AS nombreTitular   , "
			+ "SPIS.DES_PAIS AS nacionalidadTitular   , "
			+ "P1.CVE_RFC AS rfcTitular  , "
			+ "DO1.REF_CALLE AS calleTitular   , "
			...
			+ "P1.REF_TELEFONO_FIJO AS telefonoFijo   , PQ.MON_PRECIO as totalImporte   , TPM.DES_TIPO_PAGO_MENSUAL AS numPago   , "
			+ "DOV.REF_ESTADO AS ciudadFirma   , "
			+ "CONCAT(DATE_FORMAT(PSFPA.FEC_ALTA, '%d de '),ELT(MONTH(PSFPA.FEC_ALTA), \"ENERO\", \"FEBRERO\", \"MARZO\", \"ABRIL\", \"MAYO\", \"JUNIO\", \"JULIO\", \"AGOSTO\", \"SEPTIEMBRE\", \"OCTUBRE\", \"NOVIEMBRE\", \"DICIEMBRE\"),DATE_FORMAT(PSFPA.FEC_ALTA, ' de %Y')) AS fechaFirma  , "
			+ "PQ.REF_PAQUETE_NOMBRE AS nomPaquete   ,"
			...
			+ "PQ.REF_PAQUETE_DESCRIPCION AS servInclPaquete   , "
			+ "P5.REF_CORREO AS correoVelatorio   , CTE.ID_CONTRATANTE AS numeroAfiliacion   , CONCAT(IFNULL(P2.NOM_PERSONA,''),' ',IFNULL(P2.NOM_PRIMER_APELLIDO,''), ' ',IFNULL(P2.NOM_SEGUNDO_APELLIDO,'')) AS nombreSustituto  , DATE_FORMAT(P2.FEC_NAC, '%d-%m-%Y') AS fecNacSustituto  , "
			+ "P2.CVE_RFC AS rfcSustituto   , P2.REF_TELEFONO AS telefonoSustituto    , "
			+ "CONCAT(IFNULL(CONCAT(DO2.REF_CALLE,','),''),' ',IFNULL(CONCAT(DO2.NUM_EXTERIOR,','),''), ' ',IFNULL(CONCAT(DO2.NUM_INTERIOR,','),''),' ',IFNULL(CONCAT(DO2.REF_CP,','),''), ' ',IFNULL(CONCAT(DO2.REF_COLONIA,','),''),' ',IFNULL(CONCAT(DO2.REF_MUNICIPIO,','),''), ' ',IFNULL(CONCAT(DO2.REF_ESTADO,'.'),'')) AS direccionSustituto   , "
			+ "CONCAT(IFNULL(P3.NOM_PERSONA,''),' ',IFNULL(P3.NOM_PRIMER_APELLIDO,''), ' ',IFNULL(P3.NOM_SEGUNDO_APELLIDO,'')) AS nombreB1  , DATE_FORMAT(P3.FEC_NAC, '%d-%m-%Y') AS fecNacB1   , P3.CVE_RFC AS rfcB1   , P3.REF_TELEFONO AS telefonoB1   , "
			+ "CONCAT(IFNULL(CONCAT(DO3.REF_CALLE,','),''),' ',IFNULL(CONCAT(DO3.NUM_EXTERIOR,','),''), ' ',IFNULL(CONCAT(DO3.NUM_INTERIOR,','),''),' ',IFNULL(CONCAT(DO3.REF_CP,','),''), ' ',IFNULL(CONCAT(DO3.REF_COLONIA,','),''),' ',IFNULL(CONCAT(DO3.REF_MUNICIPIO,','),''), ' ',IFNULL(CONCAT(DO3.REF_ESTADO,'.'),'')) AS direccionB1  , "
			+ "CONCAT(IFNULL(P4.NOM_PERSONA,''),' ',IFNULL(P4.NOM_PRIMER_APELLIDO,''), ' ',IFNULL(P4.NOM_SEGUNDO_APELLIDO,'')) AS nombreB2   , DATE_FORMAT(P4.FEC_NAC, '%d-%m-%Y') AS fecNacB2   , "
			+ "P4.CVE_RFC AS rfcB2   , P4.REF_TELEFONO AS telefonoB2   , "
			+ "CONCAT(IFNULL(CONCAT(DO4.REF_CALLE, ','),''),' ',IFNULL(CONCAT(DO4.NUM_EXTERIOR, ','),''), ' ',IFNULL(CONCAT(DO4.NUM_INTERIOR, ','),''), ' ',IFNULL(CONCAT(DO4.REF_CP, ','),''),' ',IFNULL(CONCAT(DO4.REF_COLONIA, ','),''),' ',IFNULL(CONCAT(DO4.REF_MUNICIPIO, ','),''), ' ',IFNULL(CONCAT(DO4.REF_ESTADO, '.'),'')) AS direccionB2  "
			+ "FROM SVT_PLAN_SFPA PSFPA "
			+ "JOIN SVT_PAQUETE PQ ON PQ.ID_PAQUETE = PSFPA.ID_PAQUETE AND PQ.IND_ACTIVO = 1 "
			+ "JOIN SVC_CONTRATANTE CTE ON CTE.ID_CONTRATANTE = PSFPA.ID_TITULAR  JOIN SVC_PERSONA P1 ON P1.ID_PERSONA = CTE.ID_PERSONA "
			+ "JOIN SVC_PAIS SPIS ON P1.ID_PAIS = SPIS.ID_PAIS "
			+ "JOIN SVT_DOMICILIO DO1 ON CTE.ID_DOMICILIO = DO1.ID_DOMICILIO "
			+ "JOIN SVC_TIPO_PAGO_MENSUAL TPM  ON TPM.ID_TIPO_PAGO_MENSUAL = PSFPA.ID_TIPO_PAGO_MENSUAL "
			+ "JOIN SVC_VELATORIO VO ON VO.ID_VELATORIO = PSFPA.ID_VELATORIO "
			+ "JOIN SVT_USUARIOS US ON US.ID_USUARIO = VO.ID_USUARIO_ADMIN "
			+ "LEFT JOIN  SVC_PERSONA P5 ON P5.ID_PERSONA  = US.ID_PERSONA  JOIN SVT_DOMICILIO DO ON DO.ID_DOMICILIO = CTE.ID_DOMICILIO  "
			+ "LEFT JOIN SVT_TITULAR_BENEFICIARIOS TBSP1 ON TBSP1.ID_TITULAR_BENEFICIARIOS  = PSFPA.ID_TITULAR_SUBSTITUTO  "
			+ "LEFT JOIN SVC_PERSONA P2 ON P2.ID_PERSONA = TBSP1.ID_PERSONA  "
			+ "LEFT JOIN SVT_DOMICILIO DO2 ON DO2.ID_DOMICILIO = TBSP1.ID_DOMICILIO "
			+ "LEFT JOIN SVT_TITULAR_BENEFICIARIOS TBSP2 ON TBSP2.ID_TITULAR_BENEFICIARIOS  = PSFPA.ID_BENEFICIARIO_1 "
			+ "LEFT JOIN SVC_PERSONA P3 ON P3.ID_PERSONA = TBSP2.ID_PERSONA  "
			+ "LEFT JOIN SVT_DOMICILIO DO3 ON DO3.ID_DOMICILIO = TBSP2.ID_DOMICILIO "
			+ "LEFT JOIN SVT_TITULAR_BENEFICIARIOS TBSP3 ON TBSP3.ID_TITULAR_BENEFICIARIOS  = PSFPA.ID_BENEFICIARIO_2  "
			+ "LEFT JOIN SVC_PERSONA P4 ON P4.ID_PERSONA = TBSP3.ID_PERSONA  "
			+ "JOIN SVT_DOMICILIO DOV ON DOV.ID_DOMICILIO = VO.ID_DOMICILIO "
			+ "LEFT JOIN SVT_DOMICILIO DO4 ON DO4.ID_DOMICILIO = TBSP3.ID_DOMICILIO "
			+ "WHERE PSFPA.ID_PLAN_SFPA = #{idPlanSfpa}")
